package com.liuxu.app.gateway.filter;
...
public class AuthorizeFilter implements GlobalFilter {
    ...
    public Mono<Void> filter(ServerWebExchange exchange, GatewayFilterChain chain) {
        ...
        for (String allowUri : whiteList) {
            if (path.contains(allowUri)) {
                //放行
                return chain.filter(exchange);
            }
        }
        // 3. 获取请求头中的token,如果为空返回 401
...
//            return response.setComplete();
        ...
        try {
            ...
            if (i < 1) {
                // 5. 如果解析成功,获取token中存放的用户id
                String id = null;
                if (claimsBody != null) {
                    id = claimsBody.get("id").toString();
                }
                // 将用户id设置到请求头中 路由给其它微服务
                request.mutate().header("userId", id);
                // 认证成功放行请求
                return chain.filter(exchange);
            } else {
                log.error("token已过期");
                return writeMessage(exchange, "token已过期");
            }
        } catch (Exception e) {
